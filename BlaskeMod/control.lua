...
function GetSpawnPoint(player)
    ...
    if player.surface.can_place_entity( { name = player.character.name, position = spawnPosition }) then
        return spawnPosition
    else
        spawnPosition = player.surface.find_non_colliding_position(player.name, spawnPosition, my_config.spawn_colision_search_radius, 1)
        if spawnPosition ~= nil then
            setPlayerSpawn(player.name, spawnPosition.X, spawnPosition.Y)
            return spawnPosition
        end
    end
    return nil
